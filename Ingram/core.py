...
from .utils.error_types import IngramError
...
class Core:
    ...
    def report(self):
        ...
        if os.path.exists(results_file):
            ...
            if items:
                ...
                results_max = max(
                    [val for vul in results.values() for val in vul.values()]
                )
                ...
                for dev in results:
                    vuls = [
                        (vul_name, vul_count)
                        for vul_name, vul_count in results[dev].items()
                    ]
                    ...
                    for vul_name, vul_count in vuls:
                        ...
                        print(
                            color.green(
                                f"{vul_name:>18} | {'▥' * block_num} {vul_count}"
                            )
                        )
                print(
                    color.yellow(f"{'sum: ' + str(results_sum):>46}", "bright"),
                    flush=True,
                )
                print("-" * 46)
                print("\n")
                ...
                try:
                    if not port_scan(ip, port, self.config.timeout):
                        self.error_logger.log_error(
                            IngramError.CONNECTION, ip, port, "Port connection failed"
                        )
                        continue

                    if not (product := fingerprint(ip, port, self.config)):
                        self.error_logger.log_error(
                            IngramError.FINGERPRINT,
                            ip,
                            port,
                    ...
                    # 指纹
                    if product != "dahua":
                        # Terminal ve log dosyasına detaylı bilgi
                        skip_msg = f"Skipping {ip}:{port} - Detected as {product} (Only scanning Dahua devices)"
                        logger.warning(skip_msg)

                        # Not vulnerable dosyasına da ekleyelim
                        self.data.add_not_vulnerable(
                            [ip, str(port), f"{product} (skipped - non-Dahua)"]
                        )
                        continue
                    ...
                    self.error_logger.log_error(
                        IngramError.PROTOCOL, ip, port, f"Protocol error: {str(e)}"
                    )
    ...
    def run(self):
        ...
        try:
            ...
            # snapshot
            if not self.config.disable_snapshot:
                self.snapshot_pipeline_thread = Thread(
                    target=self.snapshot_pipeline.process,
                    args=[
                        self,
                    ],
                    daemon=True,
